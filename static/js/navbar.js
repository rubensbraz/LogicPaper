...
/**
 * Renders the navigation HTML structure into the mount point.
 * dynamically adjusts links based on the environment (GitHub Pages vs FastAPI).
 */
function renderNavbar() {
    const mountPoint = document.getElementById('navbar-mount');
    if (!mountPoint) return;

    // Environment & Link Logic
    const isGithubPages = CONFIG.env.isGithubPages;
    const suffix = isGithubPages ? '.html' : '';
    ...
    // Dashboard matches root, index.html, or trailing slash
 ...
 * Re-renders the navbar immediately to reflect changes.
 */
